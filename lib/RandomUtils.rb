...
def find_barcode(filename, desperate = false)
  zbar = Config.application_paths[:zbar]
  zbar = Config.commands[:zbar]
  zbar_d = Config.commands[:zbar_desperate]
...
def pdf_crop_tex(pdffile, dir = "./", give_error = false)
  ...
  `cd "#{dir}" && #{Config.application_paths[:pdftex]} cropped.tex`
  if $?.exitstatus != 0
...
# that path.
def temp_dir(subdir = "")
  tmp = File.join(Config.file_paths[:cache_tmp_dir], subdir)
  require 'fileutils'
...
def create_howtos(saveto, form_path = nil)
  ...
  form_path ||= Config.file_paths[:forms_howto_dir]
